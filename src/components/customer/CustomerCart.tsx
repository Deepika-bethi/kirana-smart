...
const CustomerCart = () => {
  ...
  const recommendations = products
    .filter(p => cartCategories.includes(p.category) && !cart.find(c => c.product.id === p.id) && p.quantity > 0)
    .slice(0, 3);
    ...
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-4 max-w-md mx-auto">
        <div className="glass-card p-6 text-center">
          ...
          <h2 className="font-display text-xl font-bold text-foreground">Order Confirmed!</h2>
          ...
          <p className="text-xs text-muted-foreground">Show this QR to the shopkeeper</p>
          ...
          <button
            onClick={() => { clearCart(); setShowReceipt(false); }}
            className="mt-4 w-full py-3 rounded-xl bg-primary text-primary-foreground font-display font-bold text-sm"
          >
            Done
          </button>
        </div>
    ...
    <div className="space-y-4">
      ...
      {cart.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="font-display font-bold text-foreground">Cart is empty</p>
        ...
        <>
            ...
            {cart.map(c => (
              <motion.div key={c.product.id} layout className="glass-card p-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">₹{c.product.price} each</p>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => updateCartQty(c.product.id, c.quantity - 1)} className="w-7 h-7 rounded-lg bg-muted flex items-center justify-center">
                  ...
                  <button onClick={() => removeFromCart(c.product.id)} className="w-7 h-7 rounded-lg hover:bg-destructive/10 flex items-center justify-center">
          ...
          {recommendations.length > 0 && (
            <div>
              <h3 className="font-display font-semibold text-sm text-foreground mb-2">🤝 Frequently Bought Together</h3>
          ...
          <div className="glass-card p-4 space-y-2">
            ...
            <div className="flex justify-between font-display font-bold text-foreground text-lg pt-2 border-t border-border"><span>Total</span><span>₹{total.toFixed(2)}</span></div>
          </div>

          <button
            onClick={handleCheckout}
            className="w-full py-3.5 rounded-xl bg-primary text-primary-foreground font-display font-bold text-sm hover:opacity-90 transition-all shadow-lg"
          >
