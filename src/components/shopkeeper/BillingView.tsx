...
const BillingView = () => {
  ...
  const generateBill = () => {
    if (!customerName.trim() || billItems.length === 0) {
      toast.error('Add customer name and items');
      return;
    }
    addTransaction({
      userId: 'walk-in',
      customerName,
      items: billItems.map(i => ({ productId: i.productId, productName: i.productName, quantity: i.quantity, price: i.price })),
      totalAmount: total,
      tax: gst,
    });
    setShowBill(true);
  ...
  if (showBill) {
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-lg mx-auto">
        <div className="glass-card p-6" id="printable-bill">
          <div className="text-center mb-4 pb-4 border-b border-border border-dashed">
            ...
            <p className="text-xs text-muted-foreground">Tax Invoice / बिल</p>
            <p className="text-xs text-muted-foreground mt-1">{new Date().toLocaleString('en-IN')}</p>
          </div>
          <p className="text-sm text-foreground mb-3">Customer: <strong>{customerName}</strong></p>
          ...
          <div className="pt-3 space-y-1">
            ...
            <div className="flex justify-between text-sm text-muted-foreground"><span>GST (5%)</span><span>₹{gst.toFixed(2)}</span></div>
            <div className="flex justify-between text-lg font-display font-bold text-foreground pt-2 border-t border-border"><span>Total</span><span>₹{total.toFixed(2)}</span></div>
          </div>
          <p className="text-center text-xs text-muted-foreground mt-4">Thank you! / धन्यवाद! 🙏</p>
        </div>
        <div className="flex gap-3 mt-4">
          ...
          <button onClick={() => toast.info('📱 WhatsApp sharing simulation!')} className="flex-1 py-3 rounded-xl pastel-mint-bg font-display font-bold text-sm flex items-center justify-center gap-2 text-foreground">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div className="space-y-4">
        <input
          ...
          className="w-full px-4 py-2.5 rounded-xl bg-card border border-border text-sm outline-none focus:border-primary text-foreground"
        />
        <div className="space-y-2 max-h-[60vh] overflow-y-auto">
          {filteredProducts.map(p => (
            <button
              key={p.id}
              onClick={() => addToBill(p)}
              className="w-full glass-card-hover p-3 flex items-center justify-between text-left"
            >
              <div>
                <p className="font-display font-semibold text-sm text-foreground">{p.name}</p>
                <p className="text-xs text-muted-foreground">{p.category} · {p.quantity} left</p>
              </div>
      ...
      <div className="glass-card p-5 space-y-4">
        <h3 className="font-display font-bold text-foreground flex items-center gap-2">
          <IndianRupee className="w-5 h-5" /> Current Bill
        </h3>
        <input
          value={customerName}
          onChange={e => setCustomerName(e.target.value)}
          placeholder="Customer Name (ग्राहक का नाम)"
          className="w-full px-4 py-2.5 rounded-xl bg-muted/50 border border-border text-sm outline-none focus:border-primary text-foreground"
        />
        {billItems.length === 0 ? (
          <p className="text-muted-foreground text-sm text-center py-8">Add items to create bill</p>
        ) : (
        ...
        {billItems.length > 0 && (
          <div className="pt-3 border-t border-border space-y-1">
            <div className="flex justify-between text-sm text-muted-foreground"><span>Subtotal</span><span>₹{subtotal.toFixed(2)}</span></div>
            <div className="flex justify-between text-sm text-muted-foreground"><span>GST (5%)</span><span>₹{gst.toFixed(2)}</span></div>
            <div className="flex justify-between text-lg font-display font-bold text-foreground"><span>Total</span><span>₹{total.toFixed(2)}</span></div>
